...
#station <- read.csv("Livingstone_Rain_tidy_subset.csv", header = TRUE, sep=",")
...
station$rainday <- ifelse(station$daily_rainfall_choma > 0.85,"w","d")
...
# AIC Analysis
AIC(fit_zero, fit_zero_4_harm, fit_one, fit_one_int, fit_two, fit_two_int, fit_three, fit_three_int)
 
# BIC Analysis
# Do the same for BIC
BIC(fit_zero, fit_zero_4_harm, fit_one, fit_one_int, fit_two, fit_two_int, fit_three, fit_three_int)
...
daily_rainfall_choma <- station$daily_rainfall_choma

# getting subset data for rainfall amount before fitting
amt_Rainfall <- subset(station, daily_rainfall_choma > 0.85)
head(amt_Rainfall)
str(prev1)

form_zero_2_harm <- daily_rainfall_choma ~ (cos(doy * 1 * 2 * pi/366) + sin(doy * 1 * 2 * pi/366)+
                                       cos(doy * 3 * 2 * pi/366) + sin(doy* 3 * 2 * pi/366))
...
form_zero_3_harm <- daily_rainfall_choma ~ (cos(doy * 1 * 2 * pi/366) + sin(doy * 1 * 2 * pi/366) + 
                                       cos(doy * 2 * 2 * pi/366) + sin(doy * 2 * 2 * pi/366)+
                                       cos(doy * 3 * 2 * pi/366) + sin(doy* 3 * 2 * pi/366))
...
# Look at the output from anova to see how many harmonics might be needed
# Fitting zero-order MC with 4 harmonics
form_zero_4_harm <- daily_rainfall_choma ~ (cos(doy * 1 * 2 * pi/366) + sin(doy * 1 * 2 * pi/366) + 
                                              cos(doy * 2 * 2 * pi/366) + sin(doy * 2 * 2 * pi/366)+
                                              cos(doy * 3 * 2 * pi/366) + sin(doy* 3 * 2 * pi/366)+
                                              cos(doy * 4 * 2 * pi/366) + sin(doy* 4 * 2 * pi/366))
...
# Fitting first-order MC
form_one <- daily_rainfall_choma ~ (cos(doy * 1 * 2 * pi/366) + sin(doy * 1 * 2 * pi/366) + 
                                      cos(doy * 2 * 2 * pi/366) + sin(doy * 2 * 2 * pi/366) +
                                      cos(doy * 2 * 2 * pi/366) + sin(doy * 2 * 2 * pi/366) +
                                      prev1)
...
# With * instead of + for interaction
form_one_int <- daily_rainfall_choma ~ ((cos(doy * 1 * 2 * pi/366) + sin(doy * 1 * 2 * pi/366) + 
                                           cos(doy * 2 * 2 * pi/366) + sin(doy * 2 * 2 * pi/366) +
...
# Follow a similar process to fit second order model with and without interactions
# Fitting second-order MC
form_two <- daily_rainfall_choma ~ (cos(doy * 1 * 2 * pi/366) + sin(doy * 1 * 2 * pi/366) + 
                                      cos(doy * 2 * 2 * pi/366) + sin(doy * 2 * 2 * pi/366) +
                                      cos(doy * 2 * 2 * pi/366) + sin(doy * 2 * 2 * pi/366) + prev1 +
                                      prev2)
...
# With * instead of + for interaction
form_two_int <- daily_rainfall_choma ~ ((cos(doy * 1 * 2 * pi/366) + sin(doy * 1 * 2 * pi/366) + 
                                           cos(doy * 2 * 2 * pi/366) + sin(doy * 2 * 2 * pi/366) +
...
# Fitting third-order MC
# Follow a similar process to fit third order model with and without interactions
form_three <- daily_rainfall_choma ~ (cos(doy * 1 * 2 * pi/366) + sin(doy * 1 * 2 * pi/366) + 
                                        cos(doy * 2 * 2 * pi/366) + sin(doy * 2 * 2 * pi/366) +
                                        cos(doy * 2 * 2 * pi/366) + sin(doy * 2 * 2 * pi/366) + prev1 +
                                        prev2 + prev3)

form_three_int <- daily_rainfall_choma ~ ((cos(doy * 1 * 2 * pi/366) + sin(doy * 1 * 2 * pi/366) + 
                                             cos(doy * 2 * 2 * pi/366) + sin(doy * 2 * 2 * pi/366) +
