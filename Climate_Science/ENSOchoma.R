...
form_one <- daily_rainfall_choma ~ (cos(doy * 1 * 2 * pi/366) + sin(doy * 1 * 2 * pi/366) +
                                      cos(doy * 2 * 2 * pi/366) + sin(doy * 2 * 2 * pi/366) +
                                      cos(doy * 2 * 2 * pi/366) + sin(doy * 2 * 2 * pi/366) + prev1 + ENSO)
...
#To evaluate the estimates the following codes were used:
# Estimated probability of occurrence of rain
#amt_Rainfall %>% mutate(pred_logit = predict(fitted_one, type = "daily_rainfall_choma")) -> newdf
